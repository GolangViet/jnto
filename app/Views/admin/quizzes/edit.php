<div style="margin-bottom: 24px; animation: fadeIn 0.3s ease-out;">
    ...
    <h1 style="margin: 8px 0 0; font-size: 2rem; font-weight: 700; color: #111827;">Edit Quiz Settings</h1>
</div>
...
<div class="card" style="animation: fadeIn 0.4s ease-out; padding: 28px;">
    <form action="<?= url('admin/quizzes/' . (int)$quiz['id']) ?>" method="post">
        ...
        <div style="margin-bottom: 18px;">
            ...
            <input type="text" id="title" name="title" value="<?= e($quiz['title']) ?>" required placeholder="e.g., Khám phá Nhật Bản">
        </div>
        ...
        <div style="display: none; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 18px;">
            ...
            <div>
                ...
                <label style="display: inline-flex; align-items: center; gap: 8px; cursor: pointer; margin-top: 10px;">
                    <input type="checkbox" id="allow_resume" name="allow_resume" value="1" <?= filter_var($quiz['allow_resume'] ?? true, FILTER_VALIDATE_BOOL) ? 'checked' : '' ?> style="width: auto; margin: 0;">
        ...
        <div style="display: none; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 18px;">
            <div>
                ...
                <label style="display: inline-flex; align-items: center; gap: 8px; cursor: pointer; margin-top: 10px;">
                    <input type="checkbox" id="show_result" name="show_result" value="1" <?= filter_var($quiz['show_result'] ?? true, FILTER_VALIDATE_BOOL) ? 'checked' : '' ?> style="width: auto; margin: 0;">
            ...
            <div>
                ...
                <label style="display: inline-flex; align-items: center; gap: 8px; cursor: pointer; margin-top: 10px;">
                    <input type="checkbox" id="show_correct_answer" name="show_correct_answer" value="1" <?= filter_var($quiz['show_correct_answer'] ?? true, FILTER_VALIDATE_BOOL) ? 'checked' : '' ?> style="width: auto; margin: 0;">
        ...
        <?php
        $startVal = '';
        if (!empty($quiz['start_at'])) {
            $startVal = date('Y-m-d\TH:i', strtotime($quiz['start_at']));
        }
        $endVal = '';
        if (!empty($quiz['end_at'])) {
            $endVal = date('Y-m-d\TH:i', strtotime($quiz['end_at']));
        }
        ?>
        ...
        <div style="display: none; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 24px;">
            ...
            <div>
                ...
                <input type="datetime-local" id="end_at" name="end_at" value="<?= e($endVal) ?>">
            </div>
        </div>
